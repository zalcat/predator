    ...
    if($token == $_SESSION['token']){
        ...
        if($line_pre->execute()){
            ...
            if($line_pre->num_rows>0){
                $html.= '<blockquote class="layui-elem-quote layui-quote-nm">login success</blockquote>';

            } else{
                $html.= '<blockquote class="layui-elem-quote layui-quote-nm">username or password is not exists～</blockquote>';
            }

        }else{
            $html.= '<blockquote class="layui-elem-quote layui-quote-nm">执行错误:'.$line_pre->errno.'错误信息:'.$line_pre->error.'</blockquote>';
        }


    }else{
        $html.= '<blockquote class="layui-elem-quote layui-quote-nm">csrf token error</blockquote>';
    }
}
...
<span class="layui-badge-dot layui-bg-green"></span>
    ...
    <span class="layui-badge-rim">token防爆破?</span>
...
<fieldset class="layui-elem-field layui-field-title">
  ...
  <form class="layui-form" id="bf_client" method="post" action="bf_token.php" onsubmit="return validate();">
...
<script>
  $('#test5').on('click', function(){
    layer.tips('<div  class="layui-bg-gray" style="color:#5FB878;border:8px solid #F0F0F0;"><p style="color:#2F4056;">害,这里的token真没啥用...</p></div>', '#test5');
  });</script>
